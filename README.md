...
* [运行时模块](#fpspawnersystem_fpspawnersystem)：此插件的运行时模块
	...
	- [实体数据管理器](#fpspawnersystem_entitydatamanager)：管理编辑器状态下[实体管理器](#fpspawnersystemeditor_entitymanager)生成的实体数据
		...
		- [实体数据调试](#fpspawnersystem_entitydatadebug)：提供统计数据查看、数据打印与数据清理功能，用于调试与资源优化。
	...
	- [实体数据](#fpspawnersystem_entitydata) 实体数据
		- [实体属性集](#fpspawnersystem_entityattributeset)：
		- [实体接口](#fpspawnersystem_entityinterface)：通过此接口获取实体数据，添加给生成器生成的实体Actor
		- [生成器保存数据](#) 生成器保存数据
		- [LOD](#)：
...
开发者可以将[编辑器模块](#fpspawnersystem_fpspawnersystemeditor)生成的实体数据，通过[应用散布数据](#fpspawnersystemeditor_applyscatterdata)将实体数据推给[世界分区](https://dev.epicgames.com/documentation/unreal-engine/world-partition-in-unreal-engine)管理，
但移交至[世界分区](https://dev.epicgames.com/documentation/unreal-engine/world-partition-in-unreal-engine)的实体将失去**生成器系统**的运行时功能支持。<br>

* **功能对比**：生成器系统VS世界分区

|功能项|生成器系统|世界分区|
|:-:|:-:|:-:|
|数据存储方式|未激活实体保存在内存(通过??生成器保存数据保存)|未激活实体保存在硬盘(运行时更节约内存)|
|动态编辑能力|支持运行时动态生成、修改、保存实体数据|不支持|
|扩展性|支持自定义实体行为、激活逻辑、属性管理|固定流程|

在网络游戏环境中，尽量避免通过在此模块管理大量??网格体实体，以防止因频繁同步导致网络流量过大。
...
|FPSpawnerRuntimeEntityManager|运行时实体管理器，管理自己区域内的运行时实体|
...
|FPSpawnerMeshEntityManager|生成器网格体实体管理器，管理生成器的所有??网格体实体|
...
|属性名称|类型|描述|
|:-:|:-:|:-:|
|bAutoInitSpawner|`FFPSpawnerScatterMode`|自动初始化[生成器管理子系统](#fpspawnersystem-managersubsystem)，为false时需要调用[函数库](#fpspawnersystem-functionlibrary)的函数`InitSpawnerSubsystem()`初始化[生成器管理子系统](#fpspawnersystem-managersubsystem)|
|InitDelayTime|`FName`|自动初始化[生成器管理子系统](#fpspawnersystem-managersubsystem)的延迟时间，`bAutoInitSpawner=true`时有效|
...
|MaxCalcTimePerFrame|`float`|??异步计算加载分区网格时，每帧最大计算时间(毫秒)|
|MaxFrameTime|`float`|??异步计算加载分区网格时，每帧最大时间(毫秒)|
|MaxLoadingNumberPerTick|`int32`|??读取网格单元的实体时，每帧最大加载网格单元的数量，等于0时直接异步线程处理所有加载网格单元|
...
提供了用于调试和维护的功能，主要包括数据打印与数据清理，便于开发者进行资源统计、性能分析及数据维护。

![FPSpawnerSystem_RemoveScatterData](https://github.com/FirePlume126/FP_SpawnerSystem/blob/main/Images/FPSpawnerSystem_RemoveScatterData.png)

1、打印散布数据(Print Scatter Data)，将当前加载的实体数据以表格形式输出到日志窗口，便于查看实体分布与内存占用情况。
...
2、移除散布数据(Remove Scatter Data)，`PreRemoveManagerId`输入实体管理器ID，移除对应的实体数据；`PreRemoveManagerId`输入All(不区分大小写)，移除所有数据。
...
然后基于[激活源](#fpspawnersystem_activationsource)位置进一步计算实体的新状态。
...
```c++
// 注册实体或实体移动时调用
// @param bEntityMove 实体移动时输入true，更新实体所在的网格单元
void RegisterEntity(const TWeakPtr<FFPSpawnerRuntimeEntity>& InRuntimeEntity, bool bEntityMove = false);
...
// 创建分区网格线程
template<class UserClass, typename FuncType>
static FFPSpawnerPartitionGridCalculation* CreatePartitionGridThread(const FFPSpawnerPartitionGridCalculationSettings& NewCalculationSettings, UserClass* InObject, FuncType InFunc);
...
2、更新分区网格时，传入[激活源](#fpspawnersystem_activationsource)位置数据以及包含实体的网格坐标，计算完成后返回加载与卸载的网格列表。
...
void UpdatePartitionGrid(const TArray<FVector>& NewSourceLocations, const TSet<FIntVector>& NewActivePartitionGrid, const TSet<FIntVector>& NewMovePartitionGrid);
```
...
private:
	...
	// 网格坐标
	const FIntVector GridCoord = FIntVector::ZeroValue;
