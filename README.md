...
|功能项|生成器系统|世界分区|
|:-:|:-:|:-:|
|数据存储方式|未激活实体保存在内存<br>(可以通过[生成器保存数据](#fpspawnersystem_savedata)保存在硬盘)|未激活实体保存在硬盘<br>(运行时更节约内存)|
|加载机制|异步线程计算加载/卸载网格|基于`World Partition`系统自动管理加载|
|激活逻辑|通过[激活源]控制实体激活|依赖`IWorldPartitionStreamingSourceProvider`接口控制激活|
|动态编辑能力|支持运行时动态生成、修改、保存实体数据|不支持动态编辑|
|扩展性与定制性|高度可扩展：<br>支持自定义散布策略、属性集、激活源、LOD、Tick间隔等|固定流程，扩展性有限|
...
|动态实体(如可交互对象、频繁修改状态的实体|生成器系统|支持运行时编辑、保存、LOD控制等高级功能|
|网络游戏环境|混合使用|尽量避免通过在此模块管理大量[Mesh实体数据](#fpspawnersystem_entitydata_Mesh)，以防止因频繁同步导致网络流量过大|
|需要保存运行时的实体|生成器系统|支持通过[生成器保存数据](#fpspawnersystem_savedata)序列化运行时数据并恢复运行时数据|
...
|MaxLoadingNumberPerTick|`int32`| [分区网格管理器](#fpspawnersystem_partitiongridmanager)读取分区网格单元的实体时，每帧最大加载网格单元的数量，等于0时直接异步线程处理所有加载网格单元|
...
该属性集专属于当前实体，由[实体数据](#fpspawnersystem_entitydata)中指定的`AttributeSet`类创建，并通过属性字符串映射进行反序列化与初始化。
...
浅绿色区域为`LOD 0`；浅蓝色区域为`LOD 1`；浅红色区域为`LOD 2`。
